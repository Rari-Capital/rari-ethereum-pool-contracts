...
library UniswapExchangeController {
    ...
    /**
     * @dev Swaps exact `inputAmount` of `path[0]` for at least `minOutputAmount` ETH via `path`.
     * @param inputAmount The exact input amount of `path[0]` to be swapped from.
     * @param minOutputAmount The minimum output amount of ETH to be swapped to.
     * @param path The swap path for the Uniswap V2 router.
     * @return The actual output amount.
     */
    function swapExactTokensForEth(uint256 inputAmount, uint256 minOutputAmount, address[] calldata path) external returns (uint256) {
        return UNISWAP_V2_ROUTER_02.swapExactTokensForETH(inputAmount, minOutputAmount, path, address(this), block.timestamp)[path.length - 1];
    }
}
