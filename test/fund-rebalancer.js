...
async function getFundControllerUnderlyingDydxBalance() {
  var marketId = pools["dYdX"].currencies["ETH"].marketId;
...
async function getFundControllerUnderlyingCompoundBalance() {
...
contract("RariFundController, RariFundManager", accounts => {
  it("should deposit to the fund, approve deposits to pools via RariFundController.approveToPool, and deposit to pools via RariFundController.depositToPool", async () => {
    ...
    for (const poolName of Object.keys(pools)) {
      ...
      var initialBalanceOfUnderlying = await (poolName == "Compound" ? getFundControllerUnderlyingCompoundBalance("ETH") : getFundControllerUnderlyingDydxBalance("ETH"));
      ...
      await fundControllerInstance.depositToPool(poolName == "Compound" ? 1 : 0, amountBN, { from: accounts[0] });

      // Check new pool balance
      // Accounting for dYdX and Compound losing some dust using amountBN.mul(9999).div(10000)
      var postDepositBalanceOfUnderlying = await (poolName == "Compound" ? getFundControllerUnderlyingCompoundBalance("ETH") : getFundControllerUnderlyingDydxBalance("ETH"));
  ...
  it("should withdraw half from all pools via RariFundController.withdrawFromPool", async () => {
    ...
    for (const poolName of Object.keys(pools)) {
      ...
      // TODO: Ideally, we add actually call rari-fund-rebalancer
      await fundControllerInstance.withdrawFromPool(poolName == "Compound" ? 1 : 0, amountBN.div(web3.utils.toBN(2)), { from: accounts[0] });
  ...
  it("should withdraw everything from all pools via RariFundController.withdrawAllFromPool", async () => {
    ...
    for (const poolName of Object.keys(pools)) {
      ...
      // TODO: Ideally, we add actually call rari-fund-rebalancer
      await fundControllerInstance.withdrawAllFromPool(poolName == "Compound" ? 1 : 0, { from: accounts[0] });
